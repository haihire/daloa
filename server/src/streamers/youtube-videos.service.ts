...
/** YouTube 동영상 검색 + 인기 영상(DB 누적 서빙) + 관리자 숨김 처리 */
@Injectable()
export class YoutubeVideosService implements OnModuleInit {
  ...
  async searchPopularVideos(offset = 0, limit = 0): Promise<PopularResponse> {
    ...
    try {
      const cached = await this.streamingRedis.client.get(POPULAR_CACHE_KEY);
      if (cached) {
  ...
  private async getBlockedIds(): Promise<Set<string>> {
    try {
      const ids = await this.streamingRedis.client.smembers(BLOCKED_KEY);
      return new Set(ids);
    } catch (error: unknown) {
  ...
  async blockVideo(videoId: string): Promise<void> {
    ...
    await this.streamingRedis.client.sadd(BLOCKED_KEY, videoId);
    await this.streamingRedis.client.del(POPULAR_CACHE_KEY).catch(() => {});
  ...
  async unblockVideo(videoId: string): Promise<void> {
    ...
    await this.streamingRedis.client.srem(BLOCKED_KEY, videoId);
    await this.streamingRedis.client.del(POPULAR_CACHE_KEY).catch(() => {});
  ...
  private async cachePopular(items: YoutubeVideoItem[]): Promise<void> {
    ...
    if (this.streamingRedis.readOnly) return;
    try {
      await this.streamingRedis.client.set(
        POPULAR_CACHE_KEY,
