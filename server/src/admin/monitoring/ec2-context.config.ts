...
/**
 * nest 컨테이너에서 도는 알려진 주기 작업 (운영자 직접 편집 — 크론이 바뀌면 같이 갱신).
 * AI가 CPU 스파이크 시간대를 보고 원인을 추측하는 대신, 실제로 알려진 원인이면
 * 그렇다고 단정적으로 말하고 "이상 징후"로 취급하지 않게 하기 위한 근거 데이터다.
 * 실제 스케줄: admin-inven-cron.service.ts의 @Cron 데코레이터(2시간마다, 짝수시 정각)
 */
export const SCHEDULED_JOBS_NOTE =
  '인벤 크롤 파이프라인이 nest 컨테이너에서 매 짝수시 정각(0,2,4,...,22시, KST)마다 ' +
  '실행된다. 이 시간대와 겹치는 CPU 스파이크는 이 크롤 작업이 원인일 가능성이 높다 — ' +
  '정상적으로 설계된 주기 작업이므로 그 자체만으로는 위험 신호가 아니다.';
