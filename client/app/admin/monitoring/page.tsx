...
export default function MonitoringPage() {
  ...
  useEffect(() => {
    ...
    async function load(initial = false) {
      ...
      try {
        const dashboardRes = await fetch("/api/admin/monitoring/dashboard", {
          cache: "no-store",
        });
        if (!alive) return;
  ...
  useEffect(() => {
    ...
    async function loadPageVisitSeries() {
      try {
        ...
        console.log("loadPageVisitSeries", res.status, res.ok, res);
        if (!alive || !res.ok) return;
        const dashboard = (await res.json()) as Dashboard;
        console.log("loadPageVisitSeries data", dashboard);
        setData((prev) => ({
          ...prev,
              ...
              {pageLoadLoading ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    ...
                    className={
                      pageLoadFrom !== pageLoadTo ? "cursor-pointer" : ""
                    }
                  >
        ...
        <div className="xl:relative xl:min-w-0">
          <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 xl:absolute xl:inset-0 xl:grid-cols-1 xl:grid-rows-4 xl:[&>div]:min-h-0 xl:[&>div]:overflow-y-auto">
            ...
            <div className="admin-card p-3">
              ...
              <div className="space-y-1">
                {(deviceTab === "device" ? deviceItems : browserItems)
                  .length === 0 ? (
                  ...
                  return (
                    <Fragment key={e.id}>
                      <tr
                        ...
                        onClick={() =>
                          canExpand && setExpandedErrId(expanded ? null : e.id)
                        }
                      >
